exports.handler = async (event) => {
    ...
    try {
        ...
        if (!name || name.trim().length < 2 || name.length > 50) {
            return { statusCode: 400, body: JSON.stringify({ error: "Invalid Name (2-50 chars required)" }) };
        }
        ...
        if (!packageSelected || !validPackages.includes(packageSelected)) {
            return { statusCode: 400, body: JSON.stringify({ error: "Please select a valid package" }) };
        }

        if (!location || location.trim().length < 3 || location.length > 150) {
            return { statusCode: 400, body: JSON.stringify({ error: "Invalid Location (3-150 chars required)" }) };
        }
